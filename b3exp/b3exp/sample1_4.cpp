...
int sample1_4(int argc, char *argv[])
{
  int INIT_TIME = 50;
  int width, height;
  ...
  for(int i = 0; i < INIT_TIME; i++){
	...
	subtract(tmp_img, avg_img, tmp_img);
	pow(tmp_img, 2.0, tmp_img);
	tmp_img.convertTo(tmp_img, -1, 2.0);
	sqrt(tmp_img, tmp_img);
	accumulate(tmp_img, sgm_img);
  }
  ...
  while(loop_flag){
	...
	subtract(avg_img, sgm_img, lower_img);
	subtract(lower_img, Zeta, lower_img);
	add(avg_img, sgm_img, upper_img);
	add(upper_img, Zeta, upper_img);
	...
	accumulateWeighted(frame, avg_img, B_PARAM, msk_img);
